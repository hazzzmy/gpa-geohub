...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("🔍 Land Clearing Monthly Data:", JSON.stringify(data, null, 2));
    ...
    console.error("❌ Land Clearing Monthly API error:", error);
    return NextResponse.json(
      {
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to fetch land clearing monthly statistics",
      },
      { status: 500 },
    );
  }
