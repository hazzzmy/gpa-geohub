...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("🔍 Land Preparation Monthly Data:", JSON.stringify(data, null, 2));
    ...
    console.error("❌ Land Preparation Monthly API error:", error);
    return NextResponse.json(
      {
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to fetch land preparation monthly statistics",
      },
      { status: 500 },
    );
  }
